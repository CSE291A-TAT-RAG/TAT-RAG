...
CONFIG = {
    ...
    "chunk_overlap": 80, # ~20% overlap for ~400-token chunks
...
def _load_keywords() -> List[str]:
    """Load keyword list from src/keywords.json; fallback to default if missing."""
    default = [
        "goodwill impairment",
        "non-gaap",
        "non gaap",
    ...
    kw_path = pathlib.Path(__file__).resolve().parents[1] / "src" / "keywords.json"
    try:
...
def extract_fiscal_year(doc_id: str) -> Optional[str]:
    """Best-effort fiscal year extraction from doc_id suffix like *_2019."""
...
ITEM_SLUG_RE = re.compile(r"item\s+(\d+[a-z]?)", re.I)
...
def _normalize_section_head(head: str) -> Optional[str]:
    """
    Normalize heading text to stable slugs, especially Item X/Item XA patterns.
    """
    if not head:
        return None
    # Try Item N / Item NA
    m = ITEM_SLUG_RE.search(head)
    if m:
        key = m.group(1).lower()
        return SECTION_NORMALIZATION.get(key, f"item_{key}")
    # Fallbacks for common headings
    hlow = head.lower()
    if "management" in hlow and "discussion" in hlow:
        return "item_7_mda"
    if "risk" in hlow and "factor" in hlow:
        return "item_1a_risk_factors"
    if "financial" in hlow and "statement" in hlow:
        ...
        try:
            ...
            for c in chunks:
                ...
                if HEADING_RE.match(head):
                    slug = _normalize_section_head(head)
                    current_section = slug or current_section
...
def build_chunks_for_report(pdf_path: str, doc_id: str) -> List[Dict]:
    ...
    fiscal_year = extract_fiscal_year(doc_id)

    # Collect board-related paragraphs to emit an extra summary chunk
    def build_board_summary(ps: List[ParagraphBlock]) -> List[str]:
        buf = []
        for pb in ps:
            low = pb.text.lower()
            if "board of directors" in low or ("board" in low and "director" in low):
                buf.append(pb.text)
        if not buf:
            return []
        merged = "\n\n".join(buf)
        return chunk_paragraph(merged, CONFIG["max_chunk_tokens"], CONFIG["chunk_overlap"])

    board_summaries = build_board_summary(paras)
    order = 0
    for pb in paras:
        ...
        for s in splits:
            if len(s) < CONFIG["min_chunk_length"]:
                continue
            kw_hits = _extract_keywords(s)
            md = {
                "doc_id": doc_id,
                "type": "text",
                "section_path": pb.section_path,
                "section_name": pb.section_path,
                "page": pb.page,
                "pages": [pb.page],
                "order": order,
    ...
    # Add board summary chunks (helps queries asking for full board members list)
    for bs in board_summaries:
        md = {
            "doc_id": doc_id,
            "type": "text",
            "section_path": "Board of Directors Summary",
            "section_name": "Board of Directors Summary",
            "page": 0,
            "pages": [],
            "order": order,
            "tokens": estimate_tokens(bs),
            "table_title": None,
            "row_label": None,
            "column_label": None,
            "fiscal_year": fiscal_year,
            "keywords_hit": _extract_keywords(bs),
            "hash": md5(f"{doc_id}|board_summary|{order}|{bs[:50]}")
        }
        text_chunks.append({"text": bs, "metadata": md})
        order += 1
    ...
    if CONFIG["TABLES_ENABLED"]:
        ...
        for t_idx, tb in enumerate(tables):
            md = {
                ...
                "section_name": "None",
                "table_title": pathlib.Path(tb.csv_file_name).stem,
                "row_label": None,
                "column_label": None,
                "fiscal_year": fiscal_year,
                "keywords_hit": [],
